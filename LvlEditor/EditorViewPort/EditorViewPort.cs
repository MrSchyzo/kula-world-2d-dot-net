...
namespace EditorViewPort
{
    ...
    public class Editor : Control
    {
        ...
        public byte LvlWidth 
        {
            ...
            set
            {
                if (value >= 2 && value <= 100 && lvlW > value && lvlGrid != null)
                {
                    ...
                    {
                        List<Pair<byte>> toBeEliminated = lvlGrid.Keys.ToList<Pair<byte>>().FindAll(x => x.FirstValue >= value);
        ...
        public byte LvlHeight 
        {
            ...
            set
            {
                if (value >= 2 && value <= 100 && lvlH > value && lvlGrid != null)
                {
                    ...
                    {
                        List<Pair<byte>> toBeEliminated = lvlGrid.Keys.ToList<Pair<byte>>().FindAll(x => x.SecondValue >= value);
                        KulaLevel.MapTile mt;
                        foreach (Pair<byte> p in toBeEliminated)
                        {
                            if (lvlGrid.TryGetValue(p, out mt))
        ...
        {
            TileToBeAdded.ChangeCoords(where.FirstValue, where.SecondValue);
        ...
        private bool SaveLevel(string path)
        {
            ...
            {
                if (!a.FileName.StartsWith(path))
                {
                    MessageBox.Show("Siccome il gioco attingerà i file dei livelli dalla directory " + path + " si impedisce il salvataggio diretto verso altri path.");
                    return false;
                }
                try
                {
        ...
        private void PaintDetails(PaintEventArgs e)
        {
            if (Mode == EditorMode.SelectMode)
            {
                ...
                int w = cellSize;
                int h = cellSize;
                int x = lineWidth + pointer.FirstValue * (lineWidth + cellSize);
                int y = lineWidth + pointer.SecondValue * (lineWidth + cellSize);
                g.FillRectangle(m, new Rectangle(x, y, w, h));
            }
        }
        #endregion
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            KulaLevel.MapTile mt;
            Pair<byte> p;
            if ((p = GetPairFromMouseEvent(e)) == null)
                return;
